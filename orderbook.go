...
type Limit struct { // limit is group of the ordre at the certain price level
	Price       float64
	Orders      Orders
	TotalVolume float64
}
...
// Implement the bucket of the Order of different size but sitting at the same price Level
...
func (l *Limit) DeleteOrder(o *Order) {
	for i := 0; i < len(l.Orders); i++ {
		if l.Orders[i] == o {
			...
			l.Orders = l.Orders[:len(l.Orders)-1] // Slice upto the last second bytes
		}
	}
...
func (ob *Orderbook) PlaceLimitOrder(price float64, o *Order) {
	// 1. Match the Order
	// 2. Add to the Orde  to the Order Book
